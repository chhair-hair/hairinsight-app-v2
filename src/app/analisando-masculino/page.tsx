...
export default function AnalisandoMasculinoPage() {
  const router = useRouter();
  const { quizData, updateQuizData } = useQuiz();
  
  // Cores do tema masculino (roxo)
  const colors = {
    primary: '#9333EA',
    primaryLight: '#A855F7'
  };
...
- Máscara: ${quizData.hairGoal === 'hidratacao' ? 'Hidratação profunda' : 'Fortalecimento'}
    ...
    // Simula análise IA por 3 segundos e redireciona para a versão masculina
    const timer = setTimeout(() => {
      router.push('/pre-resultado-masculino');
    }, 3000);
