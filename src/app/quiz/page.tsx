...
import { useQuiz, Gender, HairType, HairTexture, ScalpType, HairLength, WashFrequency, ProductType, HeatTools, ChemicalTreatments, HairConcerns, LifeStyle, Budget, TimeCommitment, HairGoal } from '@/lib/quiz-context';
...
export default function QuizPage() {
  ...
  const getMotivationalText = () => {
    ...
    return texts[Math.min(step - 2, texts.length - 1)] || texts[0];
  };
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return (
          <div key="step1" className="space-y-4 animate-fadeIn">
            <h2 className="text-2xl font-bold mb-6">Vamos começar! Qual é o seu gênero?</h2>
            <p className="text-white/60 mb-6">
              Isso nos ajudará a personalizar sua experiência com as cores e produtos ideais para você.
            </p>
            {[
              { value: 'feminino', label: 'Feminino', color: '#FF6F91', desc: 'Produtos e cores femininas' },
              { value: 'masculino', label: 'Masculino', color: '#9333EA', desc: 'Produtos e cores masculinas' },
            ].map((option) => (
              <button
                key={option.value}
                ...
                className={`w-full p-6 rounded-xl border-2 transition-all duration-300 text-left relative overflow-hidden group hover:scale-[1.02] active:scale-[0.98] ${
                  answers.gender === option.value ? 'border-white/30 bg-white/10' : 'border-white/10 bg-white/5 hover:border-white/20'
                }`}
                style={{
              ...
              >
                <div className="relative z-10">
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    <div className="w-4 h-4 rounded-full shadow-lg" style={{ backgroundColor: option.color }} />
                  </div>
                </div>
              </button>
            ))}
      ...
      case 2:
        return (
          <div key="step2" className="space-y-4 animate-fadeIn">
            {step > 1 && <p className="text-sm text-white/50 italic mb-4">{getMotivationalText()}</p>}
      ...
      case 12:
        return (
          <div key="step12" className="space-y-4 animate-fadeIn">
            <p className="text-sm text-white/50 italic mb-4">{getMotivationalText()}</p>
